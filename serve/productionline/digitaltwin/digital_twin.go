...
func DigitalTwinReadTcpModbus(client MQTT.Client, config conf.Config, modbusclient *modbus.ModbusClient) {
	logger.Infof("digitalTwin read tcpmodbus start")
	for {
		...
		if err != nil {
			logger.Errorf("digitalTwinRead error: %v", err)
			continue
		}
		...
		// 等待消息发送完毕
		point.Wait()
		logger.Infof("send message on topic: %s ; Message: \x1b[%dm%s\x1b[0m", "/productionLine/digitalTwin/data", constants.Cyan, sendmsg)
	}
}
...
func DigitalTwinWriteTcpModbus(client MQTT.Client, config conf.Config, modbusclient *modbus.ModbusClient) {
	...
	for {
		...
		token := client.Subscribe("/productionLine/digitalTwin/cmd", 1, func(client MQTT.Client, msg MQTT.Message) {
			...
			if err != nil {
				logger.Errorf("cmd json unmarshal error: %v", err)
				return
			}
			switch cmd.Cmd {
			case "start":
			...
			case "handAutoOn":
				...
				if err != nil {
					logger.Errorf("handAutoOn error: %v", err)
					return
				}
				logger.Infof("\x1b[%dm----------handAutoOn successful----------\x1b[0m", constants.Blue)
			case "handAutoOff":
				err := handAutoOff(modbusclient)
				if err != nil {
					logger.Errorf("handAutoOff error: %v", err)
					return
				}
				logger.Infof("\x1b[%dm----------handAutoOff successful----------\x1b[0m", constants.Blue)
			}
		})
		wait := token.Wait()
		if wait && token.Error() != nil {
			logger.Fatalf("subscribe error: %v", token.Error())
		}
	}
}
...
func DigitalTwinReadJointTcpModbus(client MQTT.Client, config conf.Config, modbusclient *modbus.ModbusClient) {
	...
	for {
		...
		if err != nil {
			logger.Errorf("digitalTwinReadJoint error: %v", err)
			continue
		}
		...
		// 等待消息发送完毕
		point.Wait()
		logger.Infof("send message on topic: %s ; Message: \x1b[%dm%s\x1b[0m", "/productionLine/digitalTwin/joint", constants.Cyan, sendmsg)
	}
}
...
func DigitalTwinReadCmdTcpModbus(client MQTT.Client, config conf.Config, modbusclient *modbus.ModbusClient) {
	logger.Infof("digitalTwin read cmd start")
	for {
		//time.Sleep(time.Millisecond * time.Duration(100))
		var sendmsg string
		...
		// 等待消息发送完毕
		point.Wait()
		logger.Infof("send message on topic: %s ; Message: \x1b[%dm%s\x1b[0m", "/productionLine/digitalTwin/cmd", constants.Cyan, sendmsg)
	}
}
